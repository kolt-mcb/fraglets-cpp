...
void setup_fraglets(fraglets& frag) {
    ...
    for (int i = 0; i< 10; i++){  // Reduced for faster testing
        frag.parse(mol);
        frag.parse("z");
    }
...
int main() {
    std::cout << "Test 1: Single-threaded (parallel=false)" << std::endl;
    {
        fraglets frag;
        setup_fraglets(frag);
        auto start = std::chrono::high_resolution_clock::now();
        frag.run(1000, 200, true, false, 1);
        auto end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        std::cout << "Time: " << duration.count() << " ms" << std::endl;
    }

    std::cout << "\nTest 2: Single-threaded via parallel path (parallel=true, threads=1)" << std::endl;
    {
        fraglets frag;
        setup_fraglets(frag);
        auto start = std::chrono::high_resolution_clock::now();
        frag.run(1000, 200, true, true, 1);
        auto end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        std::cout << "Time: " << duration.count() << " ms" << std::endl;
    }

    std::cout << "\nTest 3: Multi-threaded with 2 threads" << std::endl;
    {
        fraglets frag;
        setup_fraglets(frag);
        auto start = std::chrono::high_resolution_clock::now();
        frag.run(1000, 200, true, true, 2);
        auto end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        std::cout << "Time: " << duration.count() << " ms" << std::endl;
    }
